<template>
  <div class="countContainer">
    ...
    <div class="item">
      <span class="name">{{ $t('count.dics') }}</span>
      <span class="value">{{ dics }}</span>
    </div>
    <div class="item">
      <span class="name">{{ $t('count.cases') }}</span>
      <span class="value">{{ cases }}</span>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    onDataChange(data) {
      ...
      var json = JSON.parse(localStorage.getItem('SIMPLE_MIND_MAP_DATA'))
      
      console.log(json)
      console.log(json.root.children)
      json.root.children.forEach(dic => {
        cases += dic.children.length
      });
      dics = json.root.children.length
      this.$store.commit('setCases', cases)
      this.$store.commit('setDics', dics)
